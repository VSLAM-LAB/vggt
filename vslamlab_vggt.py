...
def viser_wrapper(
    ...
    Args:
        ...
        rgb_txt (str): file containing the image paths.
    """
    ...
    if mask_sky and rgb_txt is not None:
        conf = apply_sky_segmentation(conf, sequence_path, rgb_txt)
...
def apply_sky_segmentation(conf: np.ndarray, sequence_path: str, rgb_txt: str) -> np.ndarray:
    """
    ...
    with open(rgb_txt, 'r') as file:
        for line in file:
            ...
            image_files.append(os.path.join(sequence_path, path))
...
def main():
    ...
    image_names = []
    timestamps = []
    with open(args.rgb_txt, 'r') as file:
        for line in file:
            timestamp, path, *extra = line.strip().split(' ')
            image_names.append(os.path.join(args.sequence_path, path))
            timestamps.append(timestamp)
    ...
    with open(keyFrameTrajectory_txt, 'w') as file:
        for i_pose, q in enumerate(quaternions):
            ...
            line = f"{ts} {tx} {ty} {tz} {qx} {qy} {qz} {qw}\n"
            file.write(line)
